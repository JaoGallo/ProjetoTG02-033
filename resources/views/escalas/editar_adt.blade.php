...
@section('content')
    <div class="dashboard-container">
        ...
        @if(session('success'))
            <div class="alert alert-success"
                style="margin-bottom: 1.5rem; border-radius: 12px; border-left: 5px solid #10b981;">
        ...
        <div class="page-header header-responsive" style="display: flex; justify-content: space-between; align-items: flex-end; flex-wrap: wrap; gap: 1rem;">
            ...
            <div class="form-actions-container" style="display: flex; gap: 1rem; align-items: flex-end; flex-wrap: wrap;">
                <div class="input-group" style="width: 250px;">
                    <label style="font-size: 0.7rem; font-weight: 600; color: var(--text-secondary); display: block; margin-bottom: 0.3rem;">Selecione o Dia</label>
                    <div class="adt-input-wrapper" style="display: flex; align-items: center; gap: 0.5rem; background: #f1f5f9; padding: 0 0.75rem; border-radius: 6px; border: 1px solid var(--border-color);">
                        <i class="fa-solid fa-calendar-day" style="color: var(--primary-olive); font-size: 0.8rem;"></i>
                        <select onchange="if(this.value) window.location.href=this.value;"
                            style="background: transparent; border: none; padding: 0.5rem 0; width: 100%; outline: none; font-size: 0.85rem; font-weight: 700; color: #475569; cursor: pointer;">
                            @foreach($diasAdt as $dateStr => $dia)
                                @php
                                    $isPast = $dia['data']->isPast() && !$dia['data']->isToday();
                                    $isActive = $dateStr === $selectedDate;
                                    $url = route('escalas.edit', ['config' => $config->id, 'dia' => $dateStr]);
                                @endphp
                                <option value="{{ $isPast ? '' : $url }}" {{ $isActive ? 'selected' : '' }} {{ $isPast ? 'disabled' : '' }}>
                                    {{ $dia['data']->format('d/m/Y') }} ({{ ucfirst($dia['data']->translatedFormat('D')) }})
                                </option>
                            @endforeach
                        </select>
                    </div>
            ...
            <div class="adt-builder-layout">
                        ...
                        <div id="mon-drawer" class="drawer-content open">
                            <div style="display: flex; justify-content: flex-end; margin: 0.5rem 0;">
                                <div class="search-box">
                                    ...
                                    <input type="text" class="tropa-search" data-target="pool-monitores"
                                        placeholder="Buscar...">
                                </div>
                            </div>
                    ...
                    <div class="sidebar-section" style="margin-top: 1rem;">
                        ...
                        <div id="atdr-drawer" class="drawer-content open">
                            <div style="display: flex; justify-content: flex-end; margin: 0.5rem 0;">
                                <div class="search-box">
                                    <i class="fa-solid fa-magnifying-glass"></i>
                                    <input type="text" class="tropa-search" data-target="pool-atiradores"
                                        placeholder="Buscar...">
                                </div>
                            </div>
                ...
                <div class="adt-document-canvas" style="display: flex; flex-direction: column; gap: 2rem;">
                    ...
                    <div class="adt-paper" style="padding: 2.5rem;">
                        <div class="adt-header" style="margin-bottom: 1.5rem; border-bottom: 2px solid #333; padding-bottom: 0.5rem;">
                            <h2 style="text-align: center; font-weight: 900; font-size: 1.2rem; margin-bottom: 0.5rem;">1ª PARTE - SERVIÇOS DIÁRIOS</h2>
                            <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 1rem;">
                                @php
                                    $keys = array_keys($diasAdt);
                                    $currentIndex = array_search($selectedDate, $keys);
                                    $prevDate = $currentIndex > 0 ? $keys[$currentIndex - 1] : null;
                                    $nextDate = $currentIndex < count($keys) - 1 ? $keys[$currentIndex + 1] : null;
                                @endphp

                                <div class="day-navigation" style="display: flex; align-items: center; gap: 1rem;">
                                    @if($prevDate)
                                        <a href="{{ route('escalas.edit', ['config' => $config->id, 'dia' => $prevDate]) }}" 
                                           class="nav-btn" title="Dia Anterior"
                                           style="color: var(--primary-olive); font-size: 1.2rem; transition: transform 0.2s;">
                                            <i class="fa-solid fa-chevron-left"></i>
                                        </a>
                                    @else
                                        <span style="color: #ccc; font-size: 1.2rem;"><i class="fa-solid fa-chevron-left"></i></span>
                                    @endif

                                    <h3 style="font-size: 1rem; color: #333; margin: 0; font-weight: 700; text-transform: uppercase;">
                                        ESCALA DE SERVIÇO PARA O DIA {{ \Carbon\Carbon::parse($selectedDate)->translatedFormat('d \d\e F (l)') }}
                                    </h3>

                                    @if($nextDate)
                                        <a href="{{ route('escalas.edit', ['config' => $config->id, 'dia' => $nextDate]) }}" 
                                           class="nav-btn" title="Próximo Dia"
                                           style="color: var(--primary-olive); font-size: 1.2rem; transition: transform 0.2s;">
                                            <i class="fa-solid fa-chevron-right"></i>
                                        </a>
                                    @else
                                        <span style="color: #ccc; font-size: 1.2rem;"><i class="fa-solid fa-chevron-right"></i></span>
                                    @endif
                                </div>
                        ...
                        <div class="adt-body">
                            ...
                            <div class="adt-section">
                                ...
                                <div id="slot-comandante" class="drop-slot single-slot">
                                    <div class="slot-placeholder"
                                        style="font-size: 0.7rem; display: {{ count($selectedMonIds) > 0 ? 'none' : 'block' }};">
                                        Arraste o Comandante da Guarda aqui</div>
                            ...
                            <div class="adt-section" style="margin-top: 1.5rem;">
                                ...
                                <div id="slots-sentinelas" class="drop-slot grid-slots">
                                    <div class="slot-placeholder"
                                        style="font-size: 0.7rem; display: {{ count($selectedAtdrIds) > 0 ? 'none' : 'block' }};">
                                        Arraste as Sentinelas aqui</div>
                    ...
                    <div class="adt-paper" style="padding: 2.5rem;">
                        <div class="adt-section" style="margin-bottom: 2rem;">
                            ...
                            <textarea name="part2_instrucao" class="adt-textarea" placeholder="Digite aqui os detalhes da instrução...">{{ $config->part2_instrucao }}</textarea>
                        </div>
                        ...
                        <div class="adt-section" style="margin-bottom: 2rem;">
                            ...
                            <textarea name="part3_assuntos_gerais" class="adt-textarea" placeholder="Digite aqui os assuntos gerais e administrativos...">{{ $config->part3_assuntos_gerais }}</textarea>
                        </div>
                        ...
                        <div class="adt-section">
                            ...
                            <textarea name="part4_justica_disciplina" class="adt-textarea" placeholder="Digite aqui os assuntos de justiça e disciplina...">{{ $config->part4_justica_disciplina }}</textarea>
                        </div>
                        
                        <div style="display: flex; justify-content: flex-end; margin-top: 2rem;">
                            <button type="submit" class="btn-primary submit-btn" style="background: var(--primary-olive); border-radius: 8px;">
                                <i class="fa-solid fa-save"></i> Salvar Conteúdo do Aditamento
                            </button>
                        </div>
                    </div>
                </div>
    ...
    <style>
        ...
        /* Select do Dia */
        .adt-day-selector select:focus {
            border-color: var(--primary-olive) !important;
            box-shadow: 0 0 0 3px var(--primary-olive-light);
        }
    ...
    @if($isTodayOrFuture)
        ...
        <script>
            ...
            document.addEventListener('DOMContentLoaded', function () {
                ...
                observer.observe(document.querySelector('.adt-body'), { childList: true, subtree: true });
            });
        </script>
    @endif
